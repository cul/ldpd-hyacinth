...
RSpec.describe "Terms", type: :request, focus: true do
  ...
  #     it "successfully updates a term", focus: true do
  #       post(term_path(id: term.internal_id, format: :json), params: term_update_form_data)
  #       expect(response.status).to be(200)
  #       response_json = JSON.parse(response.body)
  #       expect(response_json).to eq({
  ...
  #     it "ignores unpermitted parameters" do
  #       post(term_path(id: term.internal_id, format: :json), params: term_update_form_data.merge({some_unpermitted_custom_field: 'some value'}))
  #       expect(response.status).to be(200)
  #       response_json = JSON.parse(response.body)
  #       expect(response_json).to eq({
  #         'internal_id' => 1,
