...
RSpec.describe 'Group Requests', type: :request do
  describe 'GET /api/v1/groups' do
    context 'when user logged in' do
      before { sign_in_user as: :group_manager }
  ...
  describe 'PATCH /api/v1/groups/:string_key' do
    ...
    context 'when logged in user is an administrator' do
      ...
      context 'when updating is_admin' do
        ...
        it 'updates records' do
          group.reload
          expect(group.is_admin).to be true
        end
    ...
    context 'when logged in user is a group manager' do
      before { sign_in_user as: :group_manager }
      ...
      context 'when updating is_admin' do
        ...
        it 'does not update record' do
          group.reload
          expect(group.is_admin).to be false
        end
  ...
  describe 'DELETE /api/v1/groups/:string_key' do
    include_examples 'requires user to have correct permissions' do
      let(:request) do
        group = FactoryBot.create(:group)
        delete "/api/v1/groups/#{group.string_key}"
      end
    end
