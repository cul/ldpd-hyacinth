...
RSpec.describe ResourceRequests::FeaturedThumbnailRegionJob, solr: true do
  ...
  describe '.create_resource_request' do
    ...
    context 'successful run' do
      ...
      it 'creates the expected ResourceRequest' do
        described_class.create_resource_request(asset, resource)
        expect(ResourceRequest.find_by(digital_object_uid: asset.uid, job_type: 'featured_thumbnail_region', status: 'pending', src_file_location: src_file_location)).to be_a(ResourceRequest)
      end
    end
    ...
    context 'when a ResourceRequest with the same job_type and digital_object_uid already exists' do
      ResourceRequest.statuses.each do |status_name, _status_number|
        ...
        it "#{expect_failure ? 'fails' : 'succeeds'} when existing ResourceRequest status is #{status_name}" do
          FactoryBot.create(:resource_request, digital_object_uid: asset.uid, job_type: 'featured_thumbnail_region', status: status_name)
          described_class.create_resource_request(asset, resource)
  ...
  describe '.eligible_object?' do
    ...
    it 'returns true for an Asset with a valid source resource and no existing featured_thumbnail_region' do
