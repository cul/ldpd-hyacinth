...
context Hyacinth::Utils::CsvImportExportUtils do
  ...
  describe ".csv_to_digital_object_data" do
    ...
    it "converts properly for a new item" do
      described_class.csv_to_digital_object_data(expected_new_item_csv_data) do |digital_object_data|
    ...
    it "converts properly for a new item csv when there is only one header row (i.e. no human-friendly header row)" do
      described_class.csv_to_digital_object_data(expected_new_item_csv_data_with_single_header) do |digital_object_data|
        expect(digital_object_data).to eq(expected_new_item)
      end
    end

    it "converts properly for a new asset" do
      described_class.csv_to_digital_object_data(expected_new_asset_csv_data) do |digital_object_data|
        expect(digital_object_data).to eq(expected_new_asset)
      end
    end

    it "converts properly for an existing item" do
      described_class.csv_to_digital_object_data(expected_existing_item_csv_data) do |digital_object_data|
    ...
    it "converts properly for multiple digital objects in the same spreadsheet" do
      ...
      described_class.csv_to_digital_object_data(expected_multiple_digital_objects_csv_data) do |digital_object_data|
    ...
    it "ignores columns with an empty hyacinth string key column header" do
      described_class.csv_to_digital_object_data(new_item_with_blank_column_header_csv_data) do |digital_object_data|
  ...
  describe ".process_internal_field_value" do
    ...
    it "raises an exception if a supplied field name does not begin with an underscore ('_')" do
      expect {
        described_class.process_internal_field_value(new_digital_object_data, 'zzz', 'field_name_that_does_not_start_with_an_underscore')
      }.to raise_error("Internal field header names must begin with an underscore ('_')")
    end

    it "properly handles a non-blank field value" do
      described_class.process_internal_field_value(new_digital_object_data, 'abc:123', '_pid')
      expect(new_digital_object_data).to eq({'pid' => 'abc:123'})
    end

    it "properly handles a non-blank field value that uses dot-notation" do
      described_class.process_internal_field_value(new_digital_object_data, 'item', '_type.string_key')
    ...
    it "properly handles a blank field value for a simple field name" do
      described_class.process_internal_field_value(new_digital_object_data, '', '_pid')
      expect(new_digital_object_data).to eq({'pid' => ''})
    end

    it "properly handles a blank field value for a multivalued field" do
      described_class.process_internal_field_value(new_digital_object_data, '', '_identifier-1')
      described_class.process_internal_field_value(new_digital_object_data, '', '_identifier-2')
    ...
    it "properly handles a blank field value for a multivalued field that uses dot-notation" do
      described_class.process_internal_field_value(new_digital_object_data, '', '_publish_target-1.string_key')
      described_class.process_internal_field_value(new_digital_object_data, '', '_publish_target-2.string_key')
      expect(new_digital_object_data).to eq(
        {
          'publish_target' => [
            {
    ...
    it "raises an exception if a 0-indexed internal field header is supplied (because headers should be 1-indexed)" do
      expect {
        described_class.process_internal_field_value(new_digital_object_data, 'value', '_publish_target-0.string_key')
  ...
  describe ".get_object_at_builder_path" do
    ...
    it "can find the value for a known path" do
      ...
      expect(described_class.get_object_at_builder_path(builder_path_test_object, builder_path)).to eq(expected)
    end

    it "returns nil when a path cannot be found" do
      builder_path = ['name', 0, 'WRONG', 0]
      expect(described_class.get_object_at_builder_path(builder_path_test_object, builder_path)).to eq(nil)
    end

    it "returns the given object if the supplied builder_path is an empty array" do
      builder_path = []
      expect(described_class.get_object_at_builder_path(builder_path_test_object, builder_path)).to eq(builder_path_test_object)
    end
  ...
  describe ".put_object_at_builder_path" do
    ...
    describe "working with non-existant target paths" do
      it "raises an exception if the given builder path doesn't exist and create_missing_path == false" do
        ...
        expect {
          described_class.put_object_at_builder_path(builder_path_test_object, non_existent_path, {'something' => 'cool'}, false)
      ...
      it "properly adds a simple top level element to a hash" do
        ...
        described_class.put_object_at_builder_path(object_to_modify, builder_path, object_to_add, true)
      ...
      it "can add two new elements to the same array at the specified indexes" do
        ...
        described_class.put_object_at_builder_path(object_to_modify, builder_path_1, obj1, true)
        described_class.put_object_at_builder_path(object_to_modify, builder_path_2, obj2, true)
      ...
      it "adds a key to an existing hash" do
        ...
        described_class.put_object_at_builder_path(two_name_object, valid_path, 'value1', true)
    ...
    describe "working with existing target paths" do
      ...
      it "replaces an existing hash element with a new value" do
        ...
        described_class.put_object_at_builder_path(two_name_object, valid_path, 'type3', false)
        expect( two_name_object ).to eq( expected )
      end
      ...
      it "replaces an existing array element with a new value" do
        ...
        described_class.put_object_at_builder_path(two_name_object, valid_path, new_value, false)
  ...
  describe ".process_dynamic_field_value" do
    ...
    it "properly handles a non-blank field value" do
      ...
      described_class.process_dynamic_field_value(new_digital_object_data, 'The', 'title-1:title_non_sort_portion', current_builder_path)
      described_class.process_dynamic_field_value(new_digital_object_data, 'Catcher in the Rye', 'title-1:title_sort_portion', current_builder_path)
      expected = {
        "dynamic_field_data" => {
          "title" => [
            {
    ...
    it "properly handles a dot-notated field" do
      current_builder_path = []
      described_class.process_dynamic_field_value(new_digital_object_data, 'Salinger, J. D.', 'name-1:name_value.value', current_builder_path)
      described_class.process_dynamic_field_value(new_digital_object_data, 'http://id.loc.gov/authorities/names/n50016589', 'name-1:name_value.uri', current_builder_path)
      expected = {
        "dynamic_field_data" => {
          "name" => [
            {
    ...
    it "properly handles a dot-notated field with a blank value" do
      current_builder_path = []
      described_class.process_dynamic_field_value(new_digital_object_data, '', 'name-1:name_value.value', current_builder_path)
      described_class.process_dynamic_field_value(new_digital_object_data, '', 'name-1:name_value.uri', current_builder_path)
      expected = {
        "dynamic_field_data" => {
          "name" => [
            {
    ...
    it "properly handles a blank field value (as an empty string)" do
      current_builder_path = []
      described_class.process_dynamic_field_value(new_digital_object_data, '', 'title-1:title_sort_portion', current_builder_path)
      expected = {
        "dynamic_field_data" => {
          "title" => [
            {
    ...
    it "raises an exception if a 0-indexed dynamic field header is supplied (because headers should be 1-indexed)" do
      current_builder_path = []
      expect {
        described_class.process_dynamic_field_value(new_digital_object_data, 'Catcher in the Rye', 'title-0:title_sort_portion', current_builder_path)
  ...
  describe ".identifiers_to_row_numbers_for_csv_data" do
    subject {
      described_class.identifiers_to_row_numbers_for_csv_data(csv_data)
    }
  ...
  describe ".parent_row_number_or_placeholder_value" do
    ...
    it "returns a numeric value if parent_identifier is a key in the map identifiers_to_row_numbers" do
      expect(described_class.parent_row_number_or_placeholder_value(parent_identifier_in_map, identifiers_to_row_numbers)).to eq(4)
    end
    it "returns a specifically-prefixed string value if parent_identifier is NOT a key in the map identifiers_to_row_numbers" do
      expect(described_class.parent_row_number_or_placeholder_value(parent_identifier_not_in_map, identifiers_to_row_numbers)).to eq(described_class::NOT_IN_SPREADSHEET_PREFIX + parent_identifier_not_in_map)
    end
  end

  describe ".prerequisite_row_map_for_csv_data" do
    subject {
      described_class.prerequisite_row_map_for_csv_data(csv_data)
    }
  ...
  describe '.convert_legacy_headers_to_modern_headers!' do
    let(:header_row) {
      [
        '_parent_digital_objects-1.pid',
    ...
    let(:expected) {
      [
        ...
        '_digital_object_type.string_key',
        '_project.string_key',
        '_import_file.main.import_type',
        '_import_file.main.import_location',
        '_import_file.main.original_file_path',
        '_import_file.service.import_type',
        '_import_file.service.import_location',
      ]
    }

    it 'converts as expected' do
      described_class.convert_legacy_headers_to_modern_headers!(header_row)
      expect(header_row).to eq(expected)
    end
  end
end
