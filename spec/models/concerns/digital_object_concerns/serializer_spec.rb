...
RSpec.describe DigitalObjectConcerns::Serializer do
  let(:digital_object) { FactoryBot.build(:digital_object_test_subclass_with_complex_data) }

  before {
    allow(digital_object).to receive(:metadata_attributes).and_return({
      'uid' => Hyacinth::DigitalObject::TypeDef::String.new,
      'group' => Hyacinth::DigitalObject::TypeDef::Group.new
    })
    allow(digital_object).to receive(:uid).and_return('unique-id-123')
    allow(digital_object).to receive(:group).and_return(FactoryBot.build(:group))

    allow(digital_object).to receive(:resource_attributes).and_return({
