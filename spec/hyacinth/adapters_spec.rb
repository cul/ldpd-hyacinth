...
describe Hyacinth::Adapters do
  ...
  context ".create_from_config" do
    it "creates a new instance of an adapter from a configuration hash" do
      expect(described_class.create_from_config(adapter_config_digital_object_search_solr)).to be_a(Hyacinth::Adapters::DigitalObjectSearchAdapter::Solr)
    end

    it "raises an error if the configuration hash does not have a type" do
      expect { described_class.create_from_config(adapter_config_without_type) }.to raise_error(Hyacinth::Exceptions::AdapterNotFoundError)
    end

    it "raises an error if the Adapter class could not be found" do
      expect { described_class.create_from_config(adapter_config_with_unresolvable_type) }.to raise_error(Hyacinth::Exceptions::AdapterNotFoundError)
    end
  end
end
