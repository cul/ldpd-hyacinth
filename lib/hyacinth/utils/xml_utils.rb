class Hyacinth::Utils::XmlUtils
  ...
  def self.replace_xml_subtrees(src_doc, dst_doc, xpaths_to_replace, namespaces={})
    ...
    xpaths_to_replace.each {|subtree_xpath|
      ...
      dst_subtree_nodes.each {|node|
        node.remove
      }
      
      # Then add the src doc nodes to the parent node of the nodes you just removed
      src_subtree_nodes.each {|node|
        parent_node.add_child(node.to_xml)
      }
      
      #dst_subtree_node.replace(src_subtree_node.to_xml)
    }
  end
