namespace :hyacinth do
  ...
  namespace :setup do
    ...
    task :config_files do
      ...
      fedora_port = ENV['fedora_port'] || 8983
      ...
      ['development', 'test'].each do |env_name|
        fedora_yml[env_name] = {
          ...
          :url => 'http://localhost:' + fedora_port + (env_name == 'test' ? '/fedora-test' : '/fedora'),
      ...
      ['development', 'test'].each do |env_name|
        hyacinth_yml[env_name] = {
          'solr_url' => 'http://localhost:' + solr_port + '/solr/' + (env_name == 'test' ? '/hyacinth-test' : '/hyacinth'),
      ...
      ['development', 'test'].each do |env_name|
        solr_yml[env_name] = {
          'url' => 'http://localhost:' + solr_port + '/solr/hydra-' + env_name
        }
      end
