  ...
  def self.perform(csv_export_id)
    ...
    CSV.open(path_to_csv_file + '.tmp', 'wb') do |csv|
      DigitalObject::Base::search_in_batches(search_params, user, 50) do |digital_object_data|
        ...
        #digital_object_type
        column_names_to_column_indexes['_digital_object_type.string_key'] = column_names_to_column_indexes.length unless column_names_to_column_indexes.has_key?('_digital_object_type.string_key')
        row[column_names_to_column_indexes['_digital_object_type.string_key']] = digital_object_data['digital_object_type']['string_key']
