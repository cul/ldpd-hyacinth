class ExportSearchResultsToCsvJob
  ...
  INTERNAL_FIELD_REGEXES_ALLOWED_ON_IMPORT = ['_pid', '_merge_dynamic_fields', '_digital_object_type.string_key', /^_parent_digital_objects-\d+\.(identifier|pid)$/, /^_identifiers-\d+$/, /^_project\.(string_key|pid)$/, /^_publish_targets-\d+\.(string_key|pid)$/, /^_parent_digital_objects-\d+\.(identifier|pid)$/]
