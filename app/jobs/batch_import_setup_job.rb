...
class BatchImportSetupJob
  ...
  def self.perform(batch_import_id)
    ...
    if batch_import.file_location.present?
      Hyacinth::Config.batch_import_storage.with_readable_tempfile(batch_import.file_location) do |csv_file|
        row_prerequisite_map = generate_index_prerequisite_map(csv_file)
        ...
        BatchImport.csv_file_to_hierarchical_json_hash(csv_file) do |json_hash_for_row, csv_row_number|
          # Create pending digital object import and link it to the parent BatchImport
          ...
          row_numbers_to_digital_object_import_ids[csv_row_number] = digital_object_import.id
        end
  ...
  #   10 => [9]    # 10 requires 9
  # }
  def self.generate_index_prerequisite_map(csv_file)
    # First, we'll generate two maps:
    ...
    # Next we want to generate a map of row numbers to parent row numbers.
    ...
    # Now we want to convert row_numbers_to_parent_row_numbers to a structure that also expresses
    ...
    # And for our final transformation, we'll create a map of import row numbers to prerequisite row
