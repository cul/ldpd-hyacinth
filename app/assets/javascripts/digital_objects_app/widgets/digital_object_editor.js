...
Hyacinth.DigitalObjectsApp.DigitalObjectEditor.prototype.init = function () {
  ...
  this.populateValuesForControlledTermFields(function () {
    ...
    if (that.digitalObject instanceof Hyacinth.DigitalObjectsApp.DigitalObject.Asset) {
      ...
      $('.regenerate-access-copy-button').on('click', function (e) {
        e.preventDefault();
        var confirmResponse = confirm('Are you sure you want to delete the current access copy and regenerate it?  This will also regenerate all derivatives.');
        if (confirmResponse) {
          //After successful deletion, return to index page
          $.ajax({
            url: '/digital_objects/' + Hyacinth.DigitalObjectsApp.params['pid'] + '/regenerate_access_copy',
            type: 'POST',
            cache: false
          }).done(function (response) {
            if (response['success']) {
              Hyacinth.addAlert('Access copy regeneration has been queued.', 'info');
            } else {
              alert(Hyacinth.unexpectedAjaxErrorMessage);
            }
          }).fail(function () {
