  ...
  module DigitalObjectData::Setters
    module DynamicFieldData
      ...
      def set_dynamic_field_data(digital_object_data, merge_dynamic_fields)
        ...
        remove_blank_fields_from_dynamic_field_data!(self.dynamic_field_data)

        # Trim whitespace for all remaining String fields
        trim_whitespace_for_dynamic_field_data!(self.dynamic_field_data)

        # Validate URI fields and raise exception if any of them are malformed
        raise_exception_if_malformed_controlled_field_data!(self.dynamic_field_data)
      end
      ...
      def remove_blank_fields_from_dynamic_field_data!(df_data)
      ...
      def trim_whitespace_for_dynamic_field_data!(df_data)
        # Step 1: Recursively handle values on lower levels
        df_data.each do |_key, value|
          if value.is_a?(Array)
