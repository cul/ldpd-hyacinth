module DigitalObjectConcerns
  module SaveBehavior
    ...
    def save_impl(opts = {})
      ...
      Hyacinth.config.lock_adapter.with_lock(opts.fetch(:lock, true) ? self.uid : nil) do |lock_object|
        ...
        Hyacinth.config.lock_adapter.with_multilock(@parent_uids_to_add + @parent_uids_to_remove) do |parent_lock_objects|
          ...
          return false unless self.process_metadata_and_assets(current_datetime)
          ...
          return false unless self.process_parent_changes

          # Step 3 (handle_preservation / handle_preservation_failure): Persist to preservation
          return false unless self.handle_preservation(current_datetime, opts[:user])

          # Step 4 (handle_publish / handle_publish_failure): Persist to preservation
          return false unless self.handle_publish(current_datetime, opts[:user])
    ...
    def process_metadata_and_assets(current_datetime)
      ...
      begin
        ...
        # Do DigitalObjectRecord change as last step in order to properly set new_record/persisted (and within a transaction)
    ...
    def handle_preservation(current_datetime, user)
      if should_preserve?
        ...
        begin
          ...
          self.publish_entries = self.publish_entries.dup.tap do |new_publish_entries|
            ...
            @publish_to.each do |publish_target_string_key|
              Hyacinth::PublishEntry.new(published_at: current_datetime, published_by: user)
            end
    ...
    def handle_publish(current_datetime, user)
      if should_publish?
