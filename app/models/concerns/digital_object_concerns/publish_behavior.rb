  ...
  module PublishBehavior
    ...
    def perform_publish_changes(publish_to: [], unpublish_from: [], publishing_user: nil)
      ...
      Hyacinth::Config.lock_adapter.with_lock(self.uid) do |lock_object|
        ...
        publish_promises = publish_to.map do |publish_target_to_publish_to|
          Concurrent::Promise.execute { perform_publish(publish_target_to_publish_to, publish_time, publishing_user) }
        end
        unpublish_promises = unpublish_from.map do |publish_target_to_unpublish_from|
          Concurrent::Promise.execute { perform_unpublish(publish_target_to_unpublish_from) }
        end
        ...
        begin
          ...
          Timeout.timeout([PUBLISH_TIMEOUT, lock_object.remaining_lock_time].min) do
            sleep 0.5 while promises.select(&:incomplete?)
          end
          ...
          publish_promises.each do |publish_promise|
            if publish_promise.fulfilled?
              new_publish_entries << value
              successful_publish_targets << value.publish_target
            else
          ...
          unpublish_promises.each do |unpublish_promise|
            if unpublish_promise.fulfilled?
              successful_unpublish_targets << value
            else
          ...
          ActiveRecord::Base.transaction do
            ...
            self.publish_entries.destroy_all(publish_target: successful_publish_targets + successful_unpublish_targets)
          ...
          if new_highest_priority_doi_eligible_publish_target.nil?
          ...
          elsif (new_doi_publish_target != current_primary_doi_publish_target) || publish_to.include?(new_doi_publish_target)
            ...
            Hyacinth::Config.external_identifier_adapter.update(self.doi, self, self.citation_location)
          end
    ...
    private
      ...
      def perform_publish(publish_target, published_at, published_by)
        ...
        raise PublishFailure, "Failed to publish to #{publish_target.string_key}. See error log for details."
      end

      def perform_unpublish(publish_target)
        publish_result, messages = Hyacinth::Config.publication_adapter.publish(publish_target, self)
        return true if publish_result

        Rails.logger.error("Failed to unpublish #{self.uid} from #{publish_target.string_key} due to the following errors: #{messages.join(', ')}")
        raise PublishFailure, "Failed to unpublish from #{publish_target.string_key}. See error log for details."
      end
