module ProjectsHelper
  def do_recursive_enabled_dynamic_field_render(project_form_builder, digital_object_type, dynamic_field_or_dynamic_field_group)
    if dynamic_field_or_dynamic_field_group.is_a?(DynamicField)
      do_recursive_enabled_dynamic_field_render_field(project_form_builder, digital_object_type, dynamic_field_or_dynamic_field_group).html_safe
    else
      do_recursive_enabled_dynamic_field_render_group(project_form_builder, digital_object_type, dynamic_field_or_dynamic_field_group).html_safe
    end
  end

  def do_recursive_enabled_dynamic_field_render_field(project_form_builder, digital_object_type, dynamic_field_or_dynamic_field_group)
    ...
    project_form_builder.fields_for(:enabled_dynamic_fields, enabled_dynamic_field) do |enabled_dynamic_field_form|
      ...
      if fieldsets.present?
        html_to_return += '<div class="col-md-4">' + enabled_dynamic_field_form.collection_select(:fieldset_ids, fieldsets, :id, :display_label, {}, class: 'form-control multiselect', 'data-multiselect-nonselected-text'.to_sym => '- No fieldsets selected -', multiple: true) + '</div>'
      end
    end
  ...
  def do_recursive_enabled_dynamic_field_render_group(project_form_builder, digital_object_type, dynamic_field_or_dynamic_field_group)
    html_to_return = '<div class="enabled_dynamic_field_row">'
