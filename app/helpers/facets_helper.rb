module FacetsHelper
  ...
  def url_for_add_facet(facet_field, facet_value)
    ...
    params_copy[:f][facet_field].push(facet_value) if ! params_copy[:f][facet_field].include?(facet_value)
  ...
  def facet_is_active?(field, value)
    return (params[:f] && params[:f][field] && params[:f][field].include?(value))
  end
