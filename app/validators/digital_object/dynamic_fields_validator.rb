...
class DigitalObject::DynamicFieldsValidator < ActiveModel::EachValidator
  ...
  private
    ...
    def errors_for_date_field(_configuration, value)
      ...
      match = NAIVE_DATE_REGEXP.match(value)
      return ['must be in YYYY-MM-DD format (with optional leading negative sign for BCE dates)'] unless match

      # We'll also run the date through a date parsing check to be safe.
      # Date.parse only words for YYYY-MM-DD dates, so we'll fill in missing parts of
      # the date with guaranteed-valid pieces in order to validate the non-missing parts.
      # Example 1 (fill in missing date and month): '1900' becomes '1900-01-01'
      # Example 2 (fill in uncertain year, and date and month): '19??' becomes '1900-01-01'
      # Note: At this time, we only support uncertainty in the year, but not in the month and day.
      filled_in_date = "#{match[1].gsub('?', '0')}-#{(match[2] || '01')}-#{(match[3] || '01')}"
      begin
        Date.parse(filled_in_date)
      rescue
