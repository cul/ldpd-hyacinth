...
function MetadataForm(props) {
  ...
  const { id, primaryProject, digitalObjectType, descriptive: initialDescriptiveMetadata } = digitalObject;
  ...
  const onSubmitHandler = () => {
    if (formType === 'edit') {
      return digitalObjectApi.update(
        id,
        { digitalObject: { descriptive: descriptiveMetadata, identifiers } },
      ).then(res => history.push(`/digital_objects/${res.data.digitalObject.uid}/metadata`));
    }

    if (formType === 'new') {
      return digitalObjectApi.create({
        digitalObject: { ...digitalObject, descriptive: descriptiveMetadata, identifiers },
      }).then(res => history.push(`/digital_objects/${res.data.digitalObject.uid}/metadata`));
    }
